...
XIRASM 的控制流是编译期行为，用来决定哪些源码会执行、哪些源码会重复执行。它不会自动生成运行时分支或循环。
...
选中的块在自己的作用域中执行。另一个块不产生输出，也不执行编译期操作。
...
没选中的 `mov` 指令不会进入输出。这是编译期指令选择，不是运行时条件分支。
...
目标查询是专用条件值。直接在 `if` 条件中使用，不要先复制到普通绑定里。
...
应让终止条件靠近循环更新逻辑，并且清晰可见。编译期循环如果停不下来会阻止汇编完成，所以 XIRASM 限制最多 1,000,000 次迭代，超限报错。
