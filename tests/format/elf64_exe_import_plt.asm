...
const imports_unused: list = elfexe_import_use64_plt(imports0, "libc.so.6", "exit")
const imports1: list = elfexe_import_use64_plt_as(imports0, "libc.so.6", "getpid", "getpid_gotplt", "getpid_plt")
...
const rela_plt_size: u64 = elfexe_import_rela_plt_size(imports1)
...
const dynamic_size: u64 = elfexe_import_dynamic_plt_size(imports1)
const ro_end_foa: u64 = dynamic_foa + dynamic_size
const data_size: u64 = elfexe_align_up(ro_end_foa - data_foa, 0x100)
const gotplt_size: u64 = elfexe_import_gotplt_size(imports1)
const plt_size: u64 = elfexe_import_plt_size(imports1)
...
plt_start:
elfexe_import_emit_plt64(imports1, plt_vaddr, gotplt_vaddr);
plt_end:
elfexe_end_segment(0x100);

elfexe_begin_segment64(".data", data_foa);
data_start:
elfexe_import_emit_gotplt64(imports1, dynamic_vaddr, plt_vaddr);
...
rela_plt_start:
elfexe_import_emit_rela_plt64(imports1, first_import_symbol);

pad_to(dynamic_foa - data_foa, 0);
dynamic_start:
elfexe_import_emit_dynamic64_plt(imports1, dynstr_vaddr, dynstr_size, dynsym_vaddr, hash_vaddr, gotplt_vaddr, rela_plt_vaddr, rela_plt_size, dynstr_needed_start);
...
defer {
    ...
    assert(plt_size == elfexe_import_plt_size(imports1));
    assert(gotplt_size == elfexe_import_gotplt_size(imports1));
    assert(rela_plt_size == elfexe_import_rela_plt_size(imports1));
    assert(dynamic_size == elfexe_import_dynamic_plt_size(imports1));
