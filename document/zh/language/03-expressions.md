...
只做输出或汇编动作的过程函数以语句方式调用，不产生表达式值。返回值函数在第 5 章介绍。
...
结构体和联合体值在第 9 章介绍。`target.bits` 和 `target.isa` 这类目标查询属于目标条件，应该直接写在 `if` 条件里，不要先复制到普通绑定中。
...
表达式无法产生明确的编译期值时，XIRASM 会报错。常见原因：
