...
const library: string = "@executable_path/libxirffi.dylib"
const library2: string = "@executable_path/libxirmath.dylib"
const imports: list = macho_import_use64(
    macho_import_use64(macho_import_new(), library, "_add7"),
    library2,
    "_sub3")
...
const command_size: u64 = macho_exe_pagezero_command_size + macho_segment_command64_size_for(2) + macho_segment_command64_size_for(1) + macho_segment_command64_size + macho_exe_dyld_info_command_size + macho_symtab_command_size + macho_dysymtab_command_size + dylinker_size + dylib_size + macho_exe_build_version_command_size + macho_exe_main_command_size
const text_foa: u64 = macho_align_up(macho_header64_size + command_size, 16)
const text_vaddr: u64 = image_base + text_foa
const code_size: u64 = 6
const stubs_foa: u64 = macho_align_up(text_foa + code_size, 4)
const stubs_vaddr: u64 = image_base + stubs_foa
const stubs_size: u64 = len(imports) * macho_import_x86_64_stub_size
...
const linkedit_foa: u64 = macho_align_up(data_foa + slots_size, page_size)
const linkedit_vaddr: u64 = image_base + linkedit_foa
const bind_foa: u64 = linkedit_foa
const bind_size: u64 = macho_import_bind_size64(imports)
...
macho_file:
macho_import_header64_x86_64(command_count, command_size);
...
macho_section64("__stubs", "__TEXT", stubs_vaddr, stubs_size, stubs_foa, 2, 0, 0, macho_s_symbol_stubs | macho_s_attr_pure_instructions | macho_s_attr_some_instructions, 0, macho_import_x86_64_stub_size, 0);
macho_segment64("__DATA", data_vaddr, linkedit_foa - data_foa, data_foa, linkedit_foa - data_foa, macho_vm_prot_read | macho_vm_prot_write, macho_vm_prot_read | macho_vm_prot_write, 1, 0);
macho_section64("__got", "__DATA", data_vaddr, slots_size, data_foa, 3, 0, 0, macho_s_non_lazy_symbol_pointers, len(imports), 0, 0);
macho_segment64("__LINKEDIT", linkedit_vaddr, macho_align_up(linkedit_size, page_size), linkedit_foa, linkedit_size, macho_vm_prot_read, macho_vm_prot_read, 0, 0);
...
entry:
emit.u8(0xe8);
emit.u32(macho_import_rel32(stubs_vaddr, text_vaddr + 5));
emit.u8(0xc3);
pad(stubs_foa - text_foa - code_size, 0);
macho_import_emit_stubs_x86_64(imports, stubs_vaddr, data_vaddr);

region.begin("__got", data_vaddr, data_foa);
slots:
macho_import_emit_slots64(imports);

region.begin("__LINKEDIT", linkedit_vaddr, linkedit_foa);
bind_data:
macho_import_emit_bind64(imports, 2);
align(8);
symbols:
...
defer {
    ...
    assert(region_file_offset(slots) == data_foa);
