...
const command_count: u64 = 4
const command_size: u64 = macho_exe_pagezero_command_size + macho_exe_text_command_size + macho_exe_main_command_size + macho_exe_build_version_command_size
...
defer {
    ...
    assert(load.u8(entry) == 0x31);
    assert(load.u8(entry + 1) == 0xc0);
    assert(load.u8(entry + 2) == 0xc3);
