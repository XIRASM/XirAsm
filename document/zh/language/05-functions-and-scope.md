...
函数封装可复用的编译期逻辑。它可以计算值、写出数据，或组合多个底层操作。
...
两种函数都使用 `fn`、参数列表和代码块。函数在汇编期间执行；声明函数本身不产生字节，只有调用过程函数，或把返回值交给输出接口时，才会影响输出。

让一个函数只承担一种职责：要么执行输出或布局操作，要么返回一个可用于表达式的值。
...
过程函数体可以包含声明、赋值、控制流、数据输出以及其他过程调用。它适合封装重复的二进制记录、指令序列、表项和格式构建步骤。
...
输出 `2a`。实参值绑定到对应参数。公开的辅助函数建议加类型注解，让约定更明确，也能在调用处发现不合适的实参。
...
函数声明只能出现在顶层，不能写在另一个函数、循环、条件块或普通块内。相关函数可以在顶层相邻排列，供后续源码调用。
...
递归在编译期执行。递归深度应控制在一定范围内，终止条件必须明确。XIRASM 拒绝超过 128 层调用的函数链，防止失控递归。
