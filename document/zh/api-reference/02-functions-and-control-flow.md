...
宏采用多行顶层声明，按源码执行顺序生效。参数是不可变 `operand`，末尾 `...name`
接收操作数列表；不支持参数注解、默认值、返回类型和 `return`。名称区分大小写，
可用点分隔；拒绝重复参数、重复精确参数数目、多个可变参数定义和语法保留名称。
精确参数数目匹配优先。已知名称参数数目不匹配报告 `MacroArityMismatch`。
空参数、不配对括号和分号分隔指令均非法。`name (expression)` 是指令调用，
`name(expression)` 是函数语法。`isa(text)` 绕过宏分派。

| API | 返回值 | 规则 |
| --- | --- | --- |
| `operand.text(op)` | `string` | 获取原始拼写，不求值。 |
| `operand.eval(op)` | 表达式值 | 使用捕获的值绑定和已有 Meta 求值器。 |
| `operand.slice(op, start, end)` | `operand` | 左闭右开字节范围，必须满足 `0 <= start <= end <= 字节长度`。 |
| `operand.split(op)` | `list` | 按顶层逗号分割，保留捕获信息；空文本返回空列表。 |

四个 API 均要求 `operand`，不会隐式接受字符串。切片允许任意合法字节边界，但
不完整表达式不能求值。空切片合法，求值时报错。操作数可存入集合并在宏返回后使用。
集合中的操作数相等比较检查片段和捕获身份；比较拼写时使用 `operand.text`。

捕获保留调用处的值，不受后续修改或被调用者同名局部变量影响。指令操作数中绑定到
`operand` 的标识符会转发原始捕获；字符串和引号内文本不替换，插入文本不再次扫描。
混合模板保留各片段的值绑定；被调用函数及位置内置函数使用第一个片段的上下文。
每次显式求值重新执行表达式；未知标识符报错，不会变成零或稍后捕获宏内变量。

宏沿用普通 Meta 类型、作用域、控制流和副作用约束，仅在 lowering 阶段按到达次数
