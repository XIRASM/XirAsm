...
`region.begin` is also not an operation that inserts bytes into the middle of
already-emitted output. It switches subsequent output to a region whose logical
origin and file offset are both chosen by the caller. The caller owns the
ordering, overlap rules, physical holes, and later header backfills. Format
facades maintain those relationships for PE, COFF, and ELF; direct custom
layouts must keep file offsets, logical addresses, and final fields separate.
...
Virtual regions are scratch output areas only. They cannot start main output
areas inside themselves: call `output.section` or `output.org` only after
returning to main output. Do not infer final storage from a virtual file
cursor. If virtual bytes need final file placement, copy them into a real
region and query that real region after layout is stable.
...
virtual regions and materialized into real file-backed regions during late
layout. For ordinary PE, COFF, and ELF output, prefer the format facades in
Chapter 14 so the facade owns section or segment records, table rows, and
backfill fields.
...
- standard PE, COFF, and ELF files should prefer the format facade so section
...
Chapters 11 through 13 are low-level layout tools. They can express where bytes
live in the file, which logical addresses they use, and when final fields are
patched. They do not automatically create PE, COFF, or ELF section records,
segment records, symbol tables, or relocation tables. Use
`format_section_begin`, `format_segment_begin`, and the relevant format APIs
when those records are needed. Combine the low-level region and finalization
tools directly only when implementing a new facade or a specialized layout that
the ordinary facade cannot cover.
