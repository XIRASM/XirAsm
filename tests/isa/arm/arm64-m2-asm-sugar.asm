...
insn_s_movz_lsl:
arm64_asm("movz x1, 42, lsl 16")

insn_s_movk:
arm64_asm("movk x2, 0x1234, lsl 48")
...
insn_s_add_imm:
arm64_asm("add x6, x7, 1")

insn_s_add_imm_lsl:
arm64_asm("add w8, w9, 1, lsl 12")

insn_s_cmp_imm:
arm64_asm("subs xzr, x10, 1")

insn_s_add_reg:
arm64_asm("add x11, x12, x13")

insn_s_add_reg_lsl:
arm64_asm("add x14, x15, x16, lsl 3")

insn_s_add_ext:
arm64_asm("add x17, x18, w19, uxtb")

insn_s_add_ext_imm:
arm64_asm("add x20, x21, w22, uxtb 2")

insn_s_orr_imm:
arm64_asm("orr x23, x24, 0xff")

insn_s_and_imm:
arm64_asm("and w25, w26, 0x80008000")

insn_s_cbz:
arm64_asm("cbz x27, s_cbz_target")
arm64_nop()
s_cbz_target:

insn_s_tbz:
arm64_asm("tbz x28, 45, s_tbz_target")
arm64_nop()
s_tbz_target:

insn_s_bcond:
arm64_asm("b.eq s_bcond_target")
arm64_nop()
s_bcond_target:

insn_s_b_back:
arm64_asm("b insn_s_b_back")
