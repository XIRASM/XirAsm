...
每个段还会遵守它所属节声明的对齐：标了 `format_executable` 的段是 16 字节，其余是 8 字节。这个对齐**同时作用在文件偏移上**，因为可加载段必须满足 `p_vaddr == p_offset (mod p_align)`——地址的低位由文件偏移决定，只挪地址会破坏加载器依赖的这个关系。结果是每个节都满足 `sh_addr % sh_addralign == 0`，这正是做对齐访问的代码所依赖的保证；代价是当某段原本会落在页内进位处时，文件里多几个字节的填充。
