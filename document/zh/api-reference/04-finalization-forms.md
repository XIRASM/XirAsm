...
引用的局部列表、映射、结构体、类型值和 operand 同样在登记时保存快照，原值后续变化不会改变快照。收尾块中的局部变量可以遮蔽捕获的同名变量，其初始化表达式仍能读取外层捕获值。operand 保留调用者绑定和捕获位置，只有显式调用 `operand.eval` 才会求值；捕获表达式中可通过 `label_addr` 在布局后解析标号。顶层 `defer` 直接引用模块级绑定时，仍按原有规则在收尾执行时查找。
