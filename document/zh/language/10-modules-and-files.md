...
如果文件字节只需要直接写入输出，可以使用 `emit.file(path)`，不必先创建中间绑定。`emit.file(path, offset, count)` 会写出一个精确范围。两种形式都复用 `fs.read_bytes` 的源文件相对路径解析和边界检查，并且不能在 `late_layout` 或 `defer` 中使用。
