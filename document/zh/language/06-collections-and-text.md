...
带引号的字面量——单引号或双引号、带不带 `b` 前缀——解码同一组很小的转义：
...
把开启引号写两遍同样得到一个引号，所以 `"a""b"` 与 `"a\"b"` 都是 `a"b`。

`\uXXXX` 必须正好四位十六进制、表示一个码点：`"\u0041"` 就是单字节 `A`，
`"\u00e9"` 是两个字节 `c3 a9`。这条存在的原因是**生成文本用这种方式写控制字符**——
Windows API 那些表就是用 `"\u0000"` 表示它们所指的 NUL 字节。

其余反斜杠都不是转义：两个字符原样保留，所以 `"\u41"`、`"\uzzzz"`、`"slash \ ok"`
都会保留反斜杠。代价是明确的：因为 `\t` 和 `\n` **是**转义，要把"反斜杠+字母"当数据写出就必须写双反斜杠，
即 `"a\\tb"` 是四个字节（`a`、`\`、`t`、`b`），而 `"a\tb"` 是三个（`a`、制表符、`b`）。

`match.tokens` 的 `quoted` 捕获解码同一组转义，因此作为值传递的指令文本与
字面量写出的文本读法一致。
