...
源码组织也遵循同一个原则：命名清晰、坐标明确、辅助函数小而直接，并选择正确的抽象层级，二进制布局才容易审查，也更容易修改。
...
当数值代表封闭集合中的一个选项时，用命名常量。多个字段组成一条记录时，用 struct。源码构造动态计划时，用 map 或 list。
...
公共辅助函数应明确说明输入。可以直接传 plan、list、map、label 或 option 时，不要依赖隐藏的可变状态。
...
普通格式接口负责描述符计数、表顺序、生成索引、偏移和常见格式不变式。

只有在实现新的格式接口，或普通接口无法表达所需布局时，才使用底层格式 include。不要仅仅因为高级 API 暴露了更多数字就切换层级；那些数字通常是普通层应继续维护的簿记。
